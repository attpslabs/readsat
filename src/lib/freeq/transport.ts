 ...
 * Adapted from freeq-app/src/irc/transport.ts
 */
...
export class Transport {
  ...
  private static MAX_RECONNECT_ATTEMPTS = 15
  ...
  connect() {
    ...
    if (!this.opts.url.startsWith('wss://')) {
      console.error('[freeq-transport] Refusing non-secure WebSocket URL')
  ...
  send(line: string) {
    if (this.ws?.readyState === WebSocket.OPEN) {
      if (this.ws.bufferedAmount > 65536) {
        console.warn('[freeq-transport] High bufferedAmount, forcing reconnect')
        this.ws.close()
        return
      }
      this.ws.send(line)
  ...
  private startHeartbeat() {
    ...
    this.heartbeatTimer = setInterval(() => {
      ...
      if (elapsed > Transport.DEAD_TIMEOUT) {
        console.warn('[freeq-transport] No data for 90s, forcing reconnect')
        this.stopHeartbeat()
        if (this.ws) {
          this.ws.close()
          this.ws = null
  ...
  private scheduleReconnect() {
    ...
    if (this.reconnectAttempts >= Transport.MAX_RECONNECT_ATTEMPTS) {
      console.warn('[freeq-transport] Max reconnect attempts reached')
      this.opts.onStateChange('disconnected')
      return
    }
